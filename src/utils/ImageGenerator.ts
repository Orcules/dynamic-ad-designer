...
export class ImageGenerator {
  ...
  private async captureElement(): Promise<string> {
    ...
    try {
      ...
      // Fixed viewport: constant width while preserving the original aspect ratio
      const originalAspectRatio = rect.height / rect.width;
      const fixedWidth = 1920; // constant width across devices
      ...
      const canvas = await html2canvas(this.previewElement, {
        ...
        width: rect.width, // keep the element's original width
        height: rect.height, // keep the element's original height
        imageTimeout: 0,
