...
export const processImages = async (
  ...
  for (let i = 0; i < uniqueImages.length; i += batchSize) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      let success = false;
      let retryCount = 0;
      const maxRetries = 1; // Reduced retries to avoid excessive memory usage
      ...
      while (retryCount < maxRetries && !success) {
        try {
          ...
          Logger.error(`Error processing image ${currentIndex + 1} (attempt ${retryCount + 1}): ${error instanceof Error ? error.message : String(error)}`);
          retryCount++;
          
          if (retryCount === maxRetries) {
            Logger.error(`Failed to process image ${currentIndex + 1} after ${maxRetries} attempts`);
