...
serve(async (req) => {
  try {
    // Parse the request body and log each step
    console.log("Parsing request body")
    const { bucket_name = "ad-images", detailed = false } = await req.json()

    // Validate bucket_name strictly (lowercase letters, digits, hyphens; max 63 chars)
    // before it is ever used in SQL statements below. This prevents SQL injection,
    // since the allowed character set cannot break out of a quoted literal.
    const BUCKET_NAME_PATTERN = /^[a-z0-9-]{1,63}$/
    if (typeof bucket_name !== 'string' || !BUCKET_NAME_PATTERN.test(bucket_name)) {
      console.error(`Rejected invalid bucket_name`)
      return new Response(
        JSON.stringify({ error: "Invalid bucket_name: must match ^[a-z0-9-]{1,63}$" }),
    ...
    // Set the access policies for the bucket
    async function ensureStoragePolicy() {
      try {
        ...
        const selectPolicy = {
          ...
          definition: "TRUE", // allow everyone
          check: "TRUE"
        }
    ...
    async function executeRawSQL(sql) {
      try {
        ...
        try {
          // Direct DB access via postgres; requires elevated permissions
    ...
    // Manually insert the bucket into the buckets table (if permitted)
